...
import { Popsicle } from 'lucide-vue-next'
...
<template>
	<div class="text-center mb-2">
		<button @click="isOpenForm = !isOpenForm" class="transition-all opacity-5 hover:opacity-100 hover:text-[#a252c8]">
	...
	<form v-if="isOpenForm" @submit="onSubmit" class="form">
		<UiInput v-model="name" placeholder="Наименование" v-bind="nameAttrs" type="text" class="input" />
		<UiInput v-model="price" placeholder="Сумма" v-bind="priceAttrs" type="text" class="input" />
		<UiInput v-model="customerEmail" placeholder="Email" v-bind="customerEmailAttrs" type="text" class="input" />
		<UiInput v-model="customerName" placeholder="Компания" v-bind="customerNameAttrs" type="text" class="input" />
